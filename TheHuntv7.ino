          ...
          if ( !isValueReceivedOnFaceExpired( f ) ) { 
            byte neighborHear = getHear(getLastValueReceivedOnFace(f));
              if (neighborHear == NOISE){
                  ...
                  if (randomdetect == 1) faceDetected++;
                  if (randomdetect == 2) faceDetected--;
                  if (faceDetected == 6) faceDetected = 0;
                  if (faceDetected == -1) faceDetected = 5;
                  gameState = DETECTED;
                  roundTimer.set(ROUND_TIME);
              }
          }
...
if (Hear == NOISE){
  ...
  if (faceDetected == 0) {setValueSentOnFace(gameState, 5);} else setValueSentOnFace(gameState, faceDetected - 1);
  if (faceDetected == 5) {setValueSentOnFace(gameState, 0);} else setValueSentOnFace(gameState, faceDetected + 1);
}
  if (roundTimer.isExpired()){
        CWIN--;
        gameState = SLAVE;
